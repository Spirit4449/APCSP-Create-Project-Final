...
class GameRoom {
  constructor(matchId, matchData, { io, db }) {
    ...
    console.log(`[GameRoom ${matchId}] Created for mode ${matchData.mode}, map ${matchData.map}`);
  }
  ...
  async addPlayer(socket, user) {
    ...
    const isParticipant = this.matchData.players.some(p => p.user_id === user.user_id);
    if (!isParticipant) {
      throw new Error('You are not a participant in this match');
    }

    // Check if player is already in the room (reconnection)
    const existingPlayer = Array.from(this.players.values()).find(p => p.user_id === user.user_id);
    if (existingPlayer) {
      ...
      // New player joining
      const matchPlayer = this.matchData.players.find(p => p.user_id === user.user_id);
      const playerData = {
        socketId: socket.id,
        ...
        // Game state
        x: 100, // Will be set by spawn logic
        y: 100,
        health: 100,
        isAlive: true,
        lastInput: Date.now(),
      ...
      console.log(`[GameRoom ${this.matchId}] Player ${user.name} joined (${this.players.size}/${this.matchData.players.length})`);
    }
    ...
    if (this.players.size === this.matchData.players.length && this.status === 'waiting') {
      this.startGame();
    }
  }
  ...
  async removePlayer(socket, user) {
    ...
    console.log(`[GameRoom ${this.matchId}] Player ${user.name} left (${this.players.size} remaining)`);
   ...
   */
  startGame() {
    console.log(`[GameRoom ${this.matchId}] Starting game with ${this.players.size} players`);
  ...
  initializeSpawnPositions() {
    const spawnPositions = {
      'team1': [{ x: 200, y: 300 }, { x: 250, y: 300 }, { x: 300, y: 300 }],
      'team2': [{ x: 800, y: 300 }, { x: 850, y: 300 }, { x: 900, y: 300 }]
    };
    ...
    for (const playerData of this.players.values()) {
      if (playerData.team === 'team1') {
        const spawn = spawnPositions.team1[team1Index % spawnPositions.team1.length];
        playerData.x = spawn.x;
        playerData.y = spawn.y;
        team1Index++;
      } else {
        const spawn = spawnPositions.team2[team2Index % spawnPositions.team2.length];
        playerData.x = spawn.x;
        playerData.y = spawn.y;
        team2Index++;
      }
  ...
  handlePlayerInput(socketId, inputData) {
    ...
    // Add timestamp and add to input buffer
    inputData.timestamp = Date.now();
    playerData.inputBuffer.push(inputData);
    
    // Keep buffer size reasonable
    if (playerData.inputBuffer.length > 10) {
      playerData.inputBuffer.shift();
    }
  ...
  handlePlayerAction(socketId, actionData) {
    ...
    console.log(`[GameRoom ${this.matchId}] Player ${playerData.name} action: ${actionData.type}`);
  ...
  processTick() {
    ...
    for (const playerData of this.players.values()) {
      ...
      if (playerData.inputBuffer.length > 0) {
        const latestInput = playerData.inputBuffer[playerData.inputBuffer.length - 1];
  ...
  processPlayerMovement(playerData, input) {
    ...
    playerData.x = Math.max(0, Math.min(1300, playerData.x));
    playerData.y = Math.max(0, Math.min(600, playerData.y));
  }
